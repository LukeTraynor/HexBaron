      ...
      if T.GetPieceInTile() is not None:
        ...
        if NoOfConnections >= ThePiece.GetConnectionsNeededToDestroy():
          ...
          if ThePiece.GetBelongsToPlayer1():
            ...
            Player2.SetPiecesInSupply(1)
          else:
            Player1VPs += ThePiece.GetVPs()
            Player1.SetPiecesInSupply(1)
...
class Player:
  ...
  def SetPiecesInSupply(self,a):
    SetPiecesInSupply = self._PiecesInSupply + a  
...
def PlayGame(Player1, Player2, Grid):
  ...
  while not (GameOver and Player1Turn):
    ...
    for Count in range (1, 4):
      Commands.append((input("Enter command: ").rstrip(" ").lower()))
    for C in Commands:
      Items = C.split(" ")
